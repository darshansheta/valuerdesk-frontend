<template>
    <div :id="section.name + '_gui_body'" class="tab-pane fade" :class="{'show active': isFirst}">
        <row-component :section="section"></row-component>
    </div>
</template>

<script>
    import RowComponent from "@/components/FormBuilder/gui/ui/RowComponent";
    import {Hooks} from '@/components/FormBuilder/gui/components/hook_lists';

    export default {
        name: "TabBodySectionLayout",
        components: {RowComponent},
        props: ['section', 'isFirst'],
        created() {
            // before hook
